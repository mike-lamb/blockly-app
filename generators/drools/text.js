...
/**
 * @fileoverview Generating Drools for text blocks.
 * @author fraser@google.com (Neil Fraser)
 */
'use strict';

goog.provide('Blockly.Drools.text');
...
Blockly.Drools['text'] = function(block) {
  // Text value.
  var code = Blockly.Drools.quote_(block.getFieldValue('TEXT'));
  return [code, Blockly.Drools.ORDER_ATOMIC];
};

Blockly.Drools['text_join'] = function(block) {
...
Blockly.Drools['text_append'] = function(block) {
  // Append to a variable in place.
  var varName = Blockly.Drools.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var argument0 = Blockly.Drools.valueToCode(block, 'TEXT',
      Blockly.Drools.ORDER_NONE) || '\'\'';
  return varName + ' = String(' + varName + ') + String(' + argument0 + ');\n';
};

Blockly.Drools['text_length'] = function(block) {
  // String length.
...
Blockly.Drools['text_isEmpty'] = function(block) {
  // Is the string null?
  var argument0 = Blockly.Drools.valueToCode(block, 'VALUE',
      Blockly.Drools.ORDER_MEMBER) || '\'\'';
  return ['!' + argument0, Blockly.Drools.ORDER_LOGICAL_NOT];
};

Blockly.Drools['text_indexOf'] = function(block) {
...
Blockly.Drools['text_getSubstring'] = function(block) {
  // Get substring.
  var text = Blockly.Drools.valueToCode(block, 'STRING',
      Blockly.Drools.ORDER_MEMBER) || '\'\'';
...
Blockly.Drools['text_changeCase'] = function(block) {
  ...
  var code;
  if (operator) {
    // Upper and lower case are functions built into Drools.
    var argument0 = Blockly.Drools.valueToCode(block, 'TEXT',
        Blockly.Drools.ORDER_MEMBER) || '\'\'';
    code = argument0 + operator;
  } else {
    // Title case is not a native Drools function.  Define one.
    var functionName = Blockly.Drools.provideFunction_(
        'text_toTitleCase',
        [ 'function ' + Blockly.Drools.FUNCTION_NAME_PLACEHOLDER_ + '(str) {',
...
Blockly.Drools['text_trim'] = function(block) {
  // Trim spaces.
  var OPERATORS = {
    LEFT: '.trimLeft()',
    RIGHT: '.trimRight()',
  ...
  var argument0 = Blockly.Drools.valueToCode(block, 'TEXT',
      Blockly.Drools.ORDER_MEMBER) || '\'\'';
  return [argument0 + operator, Blockly.Drools.ORDER_FUNCTION_CALL];
};

Blockly.Drools['text_print'] = function(block) {
  // Print statement.
  var argument0 = Blockly.Drools.valueToCode(block, 'TEXT',
      Blockly.Drools.ORDER_NONE) || '\'\'';
...
Blockly.Drools['text_prompt'] = function(block) {
  // Prompt function.
  var msg = Blockly.Drools.quote_(block.getFieldValue('TEXT'));
